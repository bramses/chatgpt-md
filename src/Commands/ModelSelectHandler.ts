...
import { fetchAvailableModels } from "./CommandUtilities";
		...
		(async () => {
			try {
				...
				const currentUrls = {
					...
					[AI_SERVICE_OPENROUTER]: frontmatter.openrouterUrl || settings.openrouterUrl || getAiApiUrls(frontmatter).openrouter,
					...
					[AI_SERVICE_ANTHROPIC]: frontmatter.anthropicUrl || settings.anthropicUrl || getAiApiUrls(frontmatter).anthropic,
