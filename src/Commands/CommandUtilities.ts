...
export async function fetchAvailableModels(
	...
	try {
		...
		if (isValidApiKey(openrouterApiKey)) {
			promises.push(
				withTimeout(
					aiService.fetchAvailableModels(urls[AI_SERVICE_OPENROUTER], openrouterApiKey, settingsService.getSettings(), "openrouter"),
					FETCH_MODELS_TIMEOUT_MS,
					[]
				)
			);
		...
		if (isValidApiKey(anthropicApiKey)) {
			promises.push(
				withTimeout(
					aiService.fetchAvailableModels(urls[AI_SERVICE_ANTHROPIC], anthropicApiKey, settingsService.getSettings(), "anthropic"),
					FETCH_MODELS_TIMEOUT_MS,
					[]
				)
			);
		...
		if (isValidApiKey(geminiApiKey)) {
			promises.push(
				withTimeout(
					aiService.fetchAvailableModels(urls[AI_SERVICE_GEMINI], geminiApiKey, settingsService.getSettings(), "gemini"),
					FETCH_MODELS_TIMEOUT_MS,
					[]
				)
			);
