  ...
  try {
    ...
    switch (provider) {
      case "openai":
        ...
        // OpenAI doesn't have a direct models endpoint in ai-sdk, so we'll use fetch
...
async function main() {
  console.log("🚀 Testing Tool Support - Fetching Models Dynamically");
  console.log("=====================================================\n");

  // Fetch models from each provider (cloud-based only, no local models)
  const allModels = [];
  ...
  // Note: Ollama and LM Studio (local models) are excluded from testing
